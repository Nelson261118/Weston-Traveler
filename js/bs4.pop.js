	...
	pop.dialog = function(opts){
		...
		if(opts.btns.length){
			...
			opts.btns.forEach(btn => {
				...
				let $btn = $('<button type="button" class="btn btn-info" class="btn '+btn.className+' pl-5 pr-5">'+btn.label+'</button>');
...
(function(pop){
	...
	pop.notice = function(content, opts){
		...
		opts = $.extend( true, {
			...
			position: 'topright', //topleft, topcenter, topright, bottomleft, bottomcenter, bottonright, center,
		...
		if(opts.autoClose){
			$el
				.append(`
					<button width="20px"; type="button" class="close" data-dismiss="alert" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				`)
